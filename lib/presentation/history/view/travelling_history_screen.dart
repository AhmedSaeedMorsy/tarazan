...
class TravellingHistoryScreen extends StatelessWidget {
  const TravellingHistoryScreen({super.key});
    ...
    return BlocBuilder<HistoryBloc, HistoryStates>(
      builder: (context, state) {
        return ConditionalBuilderRec(
          condition:
              HistoryBloc.get(context).historyTicketModel.data.isNotEmpty,
          builder: (context) {
            return Padding(
              ...
              child: ListView.separated(
                itemBuilder: (context, index) => tripItem(
                  model:
                      HistoryBloc.get(context).historyTicketModel.data[index],
                  context: context,
                ),
                ...
                itemCount:
                    HistoryBloc.get(context).historyTicketModel.data.length,
              ),
            );
          },
  ...
  Widget tripItem({
    ...
    required HistoryTicketDataModel model,
  }) {
    return Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Image(
                  image: model.travel.typeId == 1
                      ? const AssetImage(
                          AssetsManager.bus,
                        )
                      : const AssetImage(
                          AssetsManager.airplane,
                        ),
                  width: AppSize.s50.w,
                ),
                SizedBox(
